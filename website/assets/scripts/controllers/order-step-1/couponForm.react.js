...
CR.Controllers.CouponForm = React.createClass({
        ...
        return (
            <form ref="form" onSubmit={this._handleSubmit}>
                <div className="form-group">
                    <label htmlFor="coupon-code">{CR.i18nMessages["orderStep1.cartSection.coupon.label"]}</label>
                    <input type="text" className="form-control" id="coupon-code" placeholder={CR.i18nMessages["orderStep1.cartSection.coupon.field.placeholder"]} />
                    <p className="field-error" data-check="empty" />
                    <p className="field-error" id="coupon-not-found-error">{CR.i18nMessages["orderStep1.cartSection.coupon.notFoundError"]}</p>
                </div>
                <button type="submit" className="btn btn-default">{CR.i18nMessages["orderStep1.cartSection.coupon.addBtn.text"]}</button>
            </form>
            );
    },
    ...
    _handleSubmit: function(e) {
        ...
        if (this.validator.isValid()) {
            this.$addCouponBtn.enableLoading(CR.i18nMessages["orderStep1.cartSection.coupon.addBtn.loadingText"]);
