...
@Singleton
class EmailsToSendTasker @Inject()(val messagesApi: MessagesApi, val emailService: EmailService, val i18nService: I18nService) extends TimerTask with I18nSupport {
  var isRunning = false
  ...
  def run() {
    if (!isRunning) {
      ...
      for ((emailAddress, firstName, orderId) <- AccountDto.getWhoNeedUnpaidOrderReminderEmail) {
        ...
        emailService.sendUnpaidOrderReminderEmail(emailAddress, firstName, paymentUrl, i18nService.messages("email.unpaidOrderReminder.subject"))
      ...
      for ((emailAddress, firstName, orderId) <- AccountDto.getWhoNeedTheTwoDaysAfterAssessmentDeliveredEmail) {
        emailService.sendTheTwoDaysAfterAssessmentDeliveredEmail(emailAddress, firstName, i18nService.messages("email.twoDaysAfterAssessmentDelivered.subject"))
