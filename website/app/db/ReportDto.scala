...
object ReportDto {
  def getOfOrderId(orderId: Long): Option[AssessmentReport] = {
    DB.withConnection { implicit c =>
      val query = """
        select file, file_cv, li_profile_lang, job_ad_filename, added_at, added_by, d.type as doc_types, position, employer, job_ad_url, customer_comment, paid_on, custom_comment, custom_comment_cv, custom_comment_li, lang,
      ...
      val rowParser = str("file") ~ str("file_cv") ~ (str("li_profile_lang") ?) ~ (str("job_ad_filename") ?) ~ date("added_at") ~ long("added_by") ~ str("doc_types") ~ str("position") ~ str("employer") ~ (str("job_ad_url") ?) ~ (str("customer_comment") ?) ~ date("paid_on") ~ str("custom_comment") ~ str("custom_comment_cv") ~ str("custom_comment_li") ~ str("lang") ~
        ...
        case coverLetterFileName ~ cvFileName ~ liProfileLangOpt ~ jobAdFileNameOpt ~ creationDate ~ addedBy ~ docTypes ~ positionSought ~ employerSought ~ jobAdUrl ~ customerComment ~ paymentDate ~ coverLetterOverallComment ~ cvOverallComment ~ linkedinProfileOverallComment ~ languageCode ~
