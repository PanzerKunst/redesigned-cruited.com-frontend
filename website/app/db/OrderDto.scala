...
object OrderDto {
  def createTemporary(order: OrderReceivedFromFrontend): Option[Long] = {
    DB.withConnection { implicit c =>
      ...
      val query = """
      insert into documents(id, edition_id, file, file_cv, file_li, added_at, code, added_by, type, status, /* useful fields */
  ...
  def createFinalised(order: Order): Option[Long] = {
    DB.withConnection { implicit c =>
      ...
      val query = """
      insert into documents(edition_id, file, file_cv, file_li, added_at, code, added_by, type, status, paid_on, /* useful fields */
  ...
  def getOfId(id: Long): Option[Order] = {
    DB.withConnection { implicit c =>
      val query = """
        select edition_id, file, file_cv, file_li, added_at, code, added_by, type, status
        from documents
        where id = """ + id + """;"""

      Logger.info("OrderDto.getOfId():" + query)

      val optionRowParser = long("edition_id") ~ str("file") ~ str("file_cv") ~ str("file_li") ~ date("added_at") ~ str("code") ~ long("added_by") ~ str("type") ~ int("status") map {
        case editionId ~ coverLetterFileName ~ cvFileName ~ linkedinProfileFileName ~ creationDate ~ couponCode ~ accountId ~ docTypes ~ status =>
  ...
  def getOfAccountId(accountId: Long): List[Order] = {
    DB.withConnection { implicit c =>
      val query = """
        select id, edition_id, file, file_cv, file_li, added_at, code, added_by, type, status
        from documents
  ...
  def getOfAccountIdForFrontend(accountId: Long): List[FrontendOrder] = {
    DB.withConnection { implicit c =>
      val query = """
        select d.id as order_id, file, file_cv, file_li, added_at, code, added_by, type, d.status,
