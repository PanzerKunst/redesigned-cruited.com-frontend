...
class OrderDto @Inject()(db: Database, couponDto: CouponDto, accountDto: AccountDto, config: GlobalConfig) {
  ...
  def getOfId(id: Long): Option[FrontendOrder] = {
    val query = """
      select d.id as order_id, file, file_cv, file_li, li_profile_lang, added_at, type, d.status, position, employer, job_ad_url, job_ad_filename, customer_comment, paid_on, d.lang as order_lang,
        edition,
  ...
  private def processQuery(query: String): List[FrontendOrder] = {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        val linkedinProfileLanguage = rs.getString("li_profile_lang")
        val linkedinProfileLangOpt = if (rs.wasNull()) {
          None
        } else {
          Some(linkedinProfileLanguage)
        }
