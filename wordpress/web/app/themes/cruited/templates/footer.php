<footer class="content-info" role="contentinfo">
  <div class="container">
    <section id="bottom-hero" class="img-bg centered-contents">
      <div>
        <h2>Ta ett steg mot ett bättre jobb - idag!</h2>

        <a href="https://app.cruited.com/order?lang=sv" class="btn btn-primary">Starta din bedömning</a>

        <a href="/#faq">Konkreta råd, ofta inom 24 timmar,<br/>eller pengarna tillbaka</a>
      </div>
    </section>

    <section id="footer-proper">
      <section id="brand-and-social-icons">
        <h2 class="brand">Cruited</h2>
        <div>
          <a class="social-icon facebook" href="https://www.facebook.com/Cruited"></a>
          <a class="social-icon linkedin" href="https://www.linkedin.com/company/cruited"></a>
          <a class="social-icon twitter" href="https://twitter.com/cruited"></a>
        </div>
      </section>
      <section id="widgets">
        <?php dynamic_sidebar('sidebar-footer'); ?>
      </section>
    </section>
  </div>
</footer>
